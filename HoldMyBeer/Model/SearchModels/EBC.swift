...
struct EBC: CommonProperties {
    ...
    let description =
    """
    European Brewery Convention describes
    the color strength of beer in Europe.
    The color number defined by the European Brewery Convention
    is determined photometrically from a standardized mash.
    """
    let min = 0.0
    let max = 79.0
    let step = 0.1
